...
import './style.scss'
import { deleteWorkoutCycling, changeWorkoutCycling } from "../../store/action/CyclingAction";

const CyclingItems = ({date, km, comment, id}) => {
    ...
    const handleDelete = () => {
        dispatch(deleteWorkoutCycling(id))
    }
    const handleChangeWorkout = (e) => {
        setTogChange(!togChange)
        if(e.target.value === "save") {
            dispatch(changeWorkoutCycling(id,changeKm, changeComment))
        }
    }
...
CyclingItems.propTypes = {
    date: PropTypes.string,
    km: PropTypes.number,
    comment: PropTypes.string
}

export default CyclingItems;
